...
resource "random_id" "backend_alb_suffix" {
  byte_length = 2
}

resource "aws_lb" "backend_alb" {
  name               = "part3-backend-alb-${random_id.backend_alb_suffix.hex}"
  load_balancer_type = "application"
  security_groups    = [aws_security_group.alb.id]
  subnets            = [for s in aws_subnet.public : s.id]
}
...
# Backend ALB listener (dedicated URL)
  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "curl -f http://localhost:${var.backend_container_port}/api || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 10
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
  { name = "BACKEND_URL", value = "http://${aws_lb.backend_alb.dns_name}/api" },
